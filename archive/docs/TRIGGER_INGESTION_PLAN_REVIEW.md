...
**Issue**: `raw.epa_rin_prices` is **CRITICAL** (referenced in `database/models/01_raw/epa_rin_prices.sql` and `database/macros/big8_bucket_features.sql`) and now has a table definition. Ensure ingestion jobs populate it.
