...
Protocol (no hallucination):
- Verify files/imports/APIs before use.
- Enforce leakage prevention; use train-only stats; set seeds.
- Follow naming rules (`volatility_*` vs `volume_*`; never `vol_*`).
- Run sanity checks/tests; remove temp debug; avoid root clutter and keep Augment configs inside `augment/`.
...
1. **`docs/architecture/MASTER_PLAN.md`** — V15.1 architecture source of truth (UPDATED Dec 9, 2025)
2. **`AGENTS.md`** — AI assistant guardrails, Big 8 buckets, naming conventions
3. **`database/README.md`** — 8-schema layout, feature boundaries
4. **`.cursor/plans/autogluon_hybrid_implementation_c2287cb0.plan.md`** — Current implementation plan with pending tasks

## Technology Stack (DO NOT DEVIATE)

| Component | Technology | Path | Status |
|-----------|-----------|------|--------|
| **Database (Source of Truth)** | **MotherDuck Cloud** | `md:cbi_v15` | ✅ Primary (permanent) |
| **Training Workspace** | DuckDB (local Mac) | `data/duckdb/cbi_v15.duckdb` | ✅ Persistent (NOT deleted) |
| **ML Framework** | AutoGluon 1.4 | `src/training/autogluon/` | 🚧 To be created |
| **Feature Engineering** | SQL macros (AnoFox) | `database/macros/` | ✅ Active |
| **Orchestration** | Trigger.dev | `trigger/<Source>/Scripts/` | ✅ Active |
| **Training** | Mac M4 (local CPU) | Local only | ✅ Active |
| **Dashboard** | Next.js/Vercel | `dashboard/` | ✅ Active |
| **Legacy (REMOVED)** | BigQuery/Dataform/GCP | `_archive/bigquery_legacy_*` | ❌ Deprecated |
...
- **EPA**: RIN prices D3/D4/D5/D6 (weekly, FREE) ⚠️ Pending ingestion
- **EIA**: Petroleum products (ULSD wholesale, gasoline, diesel)
- **USDA FAS**: Export sales, WASDE reports
- **CFTC**: COT positioning data (weekly) ⚠️ Pending ingestion
- **ScrapeCreators**: Trump posts + 8 news buckets
- **Farm Policy News**: Real-time China/tariff policy ⚠️ Pending scraper
- **farmdoc Daily**: Academic ag economics (Scott Irwin RIN models) ⚠️ Pending scraper
- **ProFarmer**: Weather, basis, barge rates ⚠️ Pending integration
- **NOAA/INMET/Argentina SMN**: Weather data

### Invalid Symbols (DO NOT USE):
- ❌ OJ (Orange Juice) - NOT available via CME (trades on ICE)
- ❌ UL (ULSD) - Does NOT exist as separate symbol; use **HO**
- ❌ AL (Aluminum) - NOT available via CME (trades on LME)
- ❌ TY - Use **ZN** instead (TY is legacy floor symbol)
...
**Examples:**
- ✅ `databento_zl_close`, `fred_VIXCLS_close`, `epa_rin_d4_price`
- ✅ `volatility_zl_21d`, `volume_zl_21d`, `cftc_zl_managed_money_net_pct`
- ❌ `vol_zl_21d` (ambiguous - volatility or volume?)
- ❌ `volat_regime` (inconsistent)
...
CBI-V15/
├── database/
│   ├── models/              # DDL files (8 schemas) - CANONICAL
│   │   ├── 00_init/        # Schema creation
│   │   ├── 01_raw/         # Raw data tables
│   │   ├── 02_staging/     # Cleaned data
│   │   ├── 03_features/    # Engineered features
│   │   ├── 04_training/    # ML training matrices
│   │   ├── 05_assertions/  # Data quality checks
│   │   └── 06_api/         # Dashboard views
│   ├── definitions/         # DEPRECATED (see database/models/)
│   ├── macros/             # AnoFox SQL feature macros (1,428 lines)
│   └── views/              # Internal analysis views
├── src/
│   ├── training/
│   │   ├── autogluon/      # AutoGluon TabularPredictor + TimeSeriesPredictor (to be created)
│   │   └── baselines/      # Legacy LightGBM/CatBoost/XGBoost
│   ├── engines/
│   │   └── anofox/         # AnoFox SQL macro bridge
│   └── models/
│       └── tsci/           # TSci LLM agents (optional)
├── trigger/                # Trigger.dev orchestration jobs (per-source under `trigger/<Source>/Scripts/`)
├── scripts/                # Operational utilities (setup, validation)
├── config/                 # YAML/JSON configuration
├── dashboard/              # Next.js/Vercel dashboard
├── docs/
│   ├── architecture/       # MASTER_PLAN.md, design docs
│   ├── data_sources/       # DATA_SOURCES_MASTER.md
│   ├── features/           # Feature engineering docs
│   └── reference/          # BEST_PRACTICES.md
└── _archive/               # Legacy BigQuery/Dataform code (deprecated)
```
...
11. **KEEP EXPLORER CLEAN**: Place config/ignore files in their folders (`.cursor/*`, `.kilocode/*`, `augment/*`); avoid stray root files
...
- Fix Databento column: `date` → `as_of_date` in `trigger/DataBento/Scripts/collect_daily.py`
- Verify FRED references use `raw.fred_economic` (already updated in `database/macros/big8_bucket_features.sql`)
- Validate Terms of Trade / core_macro_fx (`scripts/validation/verify_core_macro_fx.py`)
- Run libomp/AutoGluon setup (`scripts/setup/install_autogluon_mac.sh`)

## Phase 1: Data Ingestion (after Phase 0)
- Create EPA RIN prices Trigger job (`trigger/EIA_EPA/Scripts/epa_rin_prices.ts`)
- Remove mock data from USDA export sales (`trigger/USDA/Scripts/ingest_export_sales.py`) and add Trigger job `usda_fas_exports.ts`
- Create CFTC COT Trigger job (`trigger/CFTC/Scripts/cftc_cot_reports.ts`)
- Create Farm Policy News scraper (`trigger/UofI_Feeds/Scripts/farmpolicynews.ts`)
- Create farmdoc Daily scraper (`trigger/UofI_Feeds/Scripts/farmdoc_daily.ts`)

## Phase 2: AutoGluon Setup (after Phase 1)
- Add AutoGluon 1.4 to requirements.txt
- Create `src/training/autogluon/` module (Tabular + TimeSeries wrappers)
- Update `src/engines/engine_registry.py` with AutoGluon models

## Phase 3: Bucket Specialists (after Phase 2)
- Create bucket training configs (8 buckets)
- Create `train_all_buckets.py` orchestrator

## Phase 4: Ensemble (after Phase 3)
- Create `database/models/04_training/ensemble_tables.sql` schema
- Update `monte_carlo_sim.py` for AutoGluon output

## Phase 5: Production Pipeline (after Phase 4)
- Create Trigger.dev training orchestration jobs
- Daily forecast pipeline to `forecasts.zl_predictions`
```
...
- **Specific table/schema names** (verify in `database/models/`)
...
- [ ] Create EPA RIN prices ingestion
      File: trigger/EIA_EPA/Scripts/epa_rin_prices.ts
      Target: raw.epa_rin_prices (date, series_id, value)
      ...
      Validation: SELECT COUNT(*) FROM raw.epa_rin_prices WHERE series_id = 'D4'
...
```bash
# Sync MotherDuck → Local (BEFORE training)
...
**Phase 0 Completed**:
- ✅ Sync script created (`scripts/sync_motherduck_to_local.py`)
- ✅ EPA RIN table created (`database/models/01_raw/epa_rin_prices.sql`)

**Phase 0 Pending Tasks**:
- Install/verify libomp + AutoGluon (`scripts/setup/install_autogluon_mac.sh`)
- Fix Databento column name (`trigger/DataBento/Scripts/collect_daily.py`: date → as_of_date)
- Validate core_macro_fx / Terms of Trade (`scripts/validation/verify_core_macro_fx.py`)
- Ensure FRED/EIA references use current tables (`raw.fred_economic`, `raw.eia_biofuels`)

**See**: `.cursor/plans/autogluon_hybrid_implementation_c2287cb0.plan.md` for complete task list

## MotherDuck → Local Sync Architecture (VERIFIED)
...
**Complete Data Flow**:
```
1. Trigger.dev jobs (cloud) → MotherDuck (SOURCE OF TRUTH - permanent)
                                    ↓
2. Before training: sync_motherduck_to_local.py → Local Mac (TRAINING WORKSPACE - persistent)
                                    ↓
3. AutoGluon training (Mac M4) → Reads/writes local workspace (100-1000x faster)
                                    ↓
4. Upload predictions → MotherDuck (forecasts.zl_predictions)
                                    ↓
5. Dashboard (Vercel) → Queries MotherDuck directly
```
...
- ✅ **MotherDuck = Source of Truth** (permanent, cloud, accessible everywhere)
- ✅ **Local Mac = Training Workspace** (persistent, fast local I/O for AutoGluon)
- ✅ **Trigger.dev writes to MotherDuck** (source of truth)
- ✅ **Sync refreshes local workspace** (before training to get latest data)
- ✅ **Dashboard reads MotherDuck** (NOT local Mac)
- ✅ **Local workspace persists** (NOT deleted after training)

**Critical Rules**:
1. **All ingestion → MotherDuck directly** (Trigger.dev jobs)
...
3. **Does the table/schema exist in database/models/?** If no, verify before creating.
