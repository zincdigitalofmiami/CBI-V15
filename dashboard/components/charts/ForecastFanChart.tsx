...
export default function ForecastFanChart({ price, forecasts }: Props) {
  ...
  useEffect(() => {
    ...
    import('lightweight-charts').then(({ createChart }) => {
      ...
      priceLine = chart.addLineSeries({ color: '#e5e7eb', lineWidth: 2.0 });
      priceLine.setData(priceSeries);

      // Forecast lines (future only)
      if (futureSeries.length > 0) {
        q50Line = chart.addLineSeries({ color: '#22c55e', lineWidth: 2, lineStyle: 0 });
        q10Line = chart.addLineSeries({ color: 'rgba(34,197,94,0.35)', lineWidth: 1, lineStyle: 1 });
        q90Line = chart.addLineSeries({ color: 'rgba(34,197,94,0.35)', lineWidth: 1, lineStyle: 1 });

        // Build lines connecting last price to first forecast, then across horizons
        const last = priceSeries[priceSeries.length - 1];
        const q50 = [{ time: last.time, value: last.value }, ...futureSeries.map((p) => ({ time: p.time, value: p.q50 }))];
        const q10 = [{ time: last.time, value: last.value }, ...futureSeries.map((p) => ({ time: p.time, value: p.q10 }))];
        const q90 = [{ time: last.time, value: last.value }, ...futureSeries.map((p) => ({ time: p.time, value: p.q90 }))];
