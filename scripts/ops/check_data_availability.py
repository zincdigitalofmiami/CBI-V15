    ...
    try:
        ...
        if count > 0:
            ...
            try:
                ...
                logger.info(f"✅ {schema}.{table}: {count:,} rows ({min_date} to {max_date})")
            except:
...
def main():
    ...
    logger.info(f"  Staging Layer: {'✅ Has data' if staging_has_data else '⚠️  No data (run staging transforms)'}")
    logger.info(f"  Features Layer: {'✅ Has data' if feature_has_data else '⚠️  No data (run feature transforms)'}")
    
    logger.info("\n🎯 Next Steps:")
    if not raw_has_data:
        logger.info("  1. Run data ingestion: python3 trigger/DataBento/Scripts/collect_daily.py")
