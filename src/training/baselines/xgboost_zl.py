#!/usr/bin/env python3
"""
XGBoost Quantile Regression for ZL
...
MODELS_DIR = ROOT_DIR / "models" / "baselines" / "xgboost"
...
def train_xgboost_quantile(
    ...
    """
    Train a single XGBoost quantile regression model.
    ...
    """
    logger.info(f"Training XGBoost for P{int(alpha*100)} quantile...")
...
def train_xgboost_for_horizon(
    train_df: pd.DataFrame,
    val_df: pd.DataFrame,
    test_df: pd.DataFrame,
    horizon_col: str,
    horizon_name: str,
) -> Tuple[Dict[float, xgb.XGBRegressor], Dict[str, Any]]:
    """
    Train XGBoost quantile models for a specific horizon.
    ...
    """
    logger.info(f"\n{'='*60}")
    logger.info(f"Training XGBoost Quantile Models for {horizon_name}")
    logger.info(f"{'='*60}")
    ...
    for alpha in QUANTILES:
        ...
        # Save model
        model_path = MODELS_DIR / f"xgboost_zl_{horizon_name}_{quantile_name}.json"
        model.save_model(str(model_path))
        logger.info(f"    ✅ Saved to {model_path}")

    # Save combined artifact with all quantiles
    combined_path = MODELS_DIR / f"xgboost_zl_{horizon_name}_all_quantiles.pkl"
    joblib.dump(
        {
...
def main():
    """Train XGBoost quantile models for all horizons."""
    logger.info("=" * 70)
    logger.info("XGBoost Quantile Training for ZL")
    logger.info("=" * 70)
    ...
    for horizon_col, horizon_name in HORIZONS.items():
        ...
        try:
            models, metrics = train_xgboost_for_horizon(
