...
export const databentoIngestJob = task({
  ...
  run: async (payload: { symbols?: string[]; daysBack?: number }, { ctx }) => {
    ...
    console.log(
      `[Databento] Starting ingestion for ${symbols.length} symbols: ${symbols.join(", ")}`,
    );
    ...
    try {
      ...
      const success =
        stdout.includes("✅") || stdout.includes("SUCCESS") || !stdout.includes("ERROR");
...
// NOTE: Scheduled via daily orchestrator, no individual schedules needed
