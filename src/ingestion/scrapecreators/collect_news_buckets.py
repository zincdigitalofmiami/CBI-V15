...
- Use MotherDuck MERGE to keep ingestion idempotent (no duplicates).
...
# Add local script and bucket paths for imports
SCRIPT_DIR = Path(__file__).resolve().parent
PROJECT_ROOT = SCRIPT_DIR.parents[3]
sys.path.insert(0, str(SCRIPT_DIR))
sys.path.insert(0, str(SCRIPT_DIR / "buckets"))
sys.path.insert(0, str(PROJECT_ROOT))

from buckets import (  # type: ignore
    collect_biofuel_policy,
    collect_china_demand,
    collect_tariffs_trade_policy,
    collect_trump_truth_social,
)
from sentiment_calculator import calculate_sentiment_finbert
