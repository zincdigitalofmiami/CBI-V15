...
def process_disaggregated_report(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Extract base commodity name before " - EXCHANGE" suffix
    df["base_name"] = (
        df["Market_and_Exchange_Names"].str.upper().str.split(" - ").str[0]
    )
    df["symbol"] = df["base_name"].map(SYMBOL_MAPPING)
    ...
    processed = df[
        [
            ...
            "Swap__Positions_Short_All",  # Note: CFTC source has double underscore
...
def ingest_cot_data(start_year: int, end_year: int):
    ...
    if all_disagg_data:
        ...
        # Register DataFrame and insert with explicit columns
        print(f"\n💾 Inserting disaggregated data into database...")
        con.register("staging_disagg", combined_disagg)

        # Delete existing rows for this date range (idempotent upsert)
        min_date = combined_disagg["report_date"].min()
        max_date = combined_disagg["report_date"].max()
        con.execute(
            f"""
            DELETE FROM raw.cftc_cot 
            WHERE report_date >= '{min_date}' AND report_date <= '{max_date}'
        """
        )

        # Insert with explicit columns (source/ingested_at use DDL defaults)
        con.execute(
            """
            INSERT INTO raw.cftc_cot (
    ...
    if all_tff_data:
        ...
        con.register("staging_tff", combined_tff)

        # Delete existing rows for this date range (idempotent upsert)
        min_date = combined_tff["report_date"].min()
        max_date = combined_tff["report_date"].max()
        con.execute(
            f"""
            DELETE FROM raw.cftc_cot_tff 
            WHERE report_date >= '{min_date}' AND report_date <= '{max_date}'
        """
        )
        ...
        con.execute(
            """
            INSERT INTO raw.cftc_cot_tff (
                report_date, symbol, open_interest,
                dealer_long, dealer_short, asset_mgr_long, asset_mgr_short,
                lev_money_long, lev_money_short,
                other_rept_long, other_rept_short, nonrept_long, nonrept_short,
                dealer_net, asset_mgr_net, lev_money_net, other_rept_net, nonrept_net,
                lev_money_net_pct_oi, asset_mgr_net_pct_oi
            )
            SELECT 
                report_date, symbol, open_interest,
                dealer_long, dealer_short, asset_mgr_long, asset_mgr_short,
