...
class AnofoxBridge:
    ...
    def __init__(self, duckdb_path: Optional[Path] = None):
        ...
        if not self.duckdb_path.exists():
            raise FileNotFoundError(f"DuckDB database not found at {self.duckdb_path}")
    ...
    def generate_baseline(self, table_name: str, method: str = "AutoETS", horizon: int = 30) -> pd.DataFrame:
        """
    ...
    def _calculate_basic_metrics(self, actual: pd.DataFrame, predicted: pd.DataFrame) -> Dict:
