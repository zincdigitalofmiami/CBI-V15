...
- ✅ **Long-horizon daily tables**: `PARTITION BY DATE_TRUNC(date, MONTH)`
- ✅ **Clustering**: `CLUSTER BY symbol` (or relevant key) where applicable
- ✅ **No Joins**: Skeleton structure = table definitions only
- ✅ **Master Join**: `training.daily_ml_matrix` joins all features
...
**Partitioning**: `PARTITION BY DATE_TRUNC(date, MONTH)` (or `report_date` for USDA where applicable)

---

### 2. `staging` Dataset (Staging Data)

**Tables**:
- `market_daily` - Staged daily OHLCV with forward-fill
- `fred_macro_panel` - FRED series with forward-fill, daily interpolation
- `usda_reports_panel` - Staged USDA reports
- `cftc_positions` - CFTC COT positions by category
- `eia_biofuels_panel` - Staged EIA biofuels data
...
**Partitioning**: `PARTITION BY DATE_TRUNC(date, MONTH)` (or `report_date` for USDA)
