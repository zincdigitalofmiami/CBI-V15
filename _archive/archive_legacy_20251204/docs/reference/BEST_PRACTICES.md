...
### Architecture
10. **Python ETL First** - All ETL transformations in Python + BigQuery SQL, version controlled (no Dataform)
...
### Pre-Work Validation
- **NEVER start a task from a plan without verification** - Always review and audit the issue first to ensure the task still needs to be done
...
- Run feature/math validation (`python scripts/validation/validate_feature_math.py`)
...
- Remove temporary resources (temporary files, test data)
- Review costs (monthly GCP billing audits)
- Update documentation (when code changes)

### File Management
- **Temporary MD files**: Delete only after task completion (allow time for review, not immediately after creation)
- NEVER create MD files unless it's a plan (use mcp_create_plan)
- Keep repo tidy - no clutter, no duplicates, no orphaned files
