#!/usr/bin/env python3
"""
FRED Financial Conditions & Risk Ingestion - Mac-only, manual run

Bucket: fred_financial_conditions

Pulls key financial conditions and risk FRED series into:
  - raw.fred_economic (date, series_id, value)
via staging tables:
  - raw_staging.fred_financial_conditions_<run_id>
"""
...
FIN_COND_SERIES: List[str] = [
    "VIXCLS",      # VIX index
    "NFCI",        # Chicago Fed National Financial Conditions Index
    "NFCILEVERAGE",  # NFCI leverage subindex
    "BAAFFM",      # BAA - Fed funds spread
    "BAMLH0A0HYM2",  # Optional: HY credit spread (if accessible in your plan)
]


def get_last_loaded_date() -> date:
    """
    Get the last loaded date across the financial conditions series in raw.fred_economic.
    If none found, return a default backfill start date.
    """
    client = get_client()
    series_list = ",".join(f"'{s}'" for s in FIN_COND_SERIES)
    query = f"""
    ...
    try:
        ...
        if pd.notna(last):
            ...
            logger.info(f"Last financial conditions date in {TABLE_ID}: {last_dt}")
            return last_dt
    except Exception as e:
        logger.warning(f"Could not determine last financial conditions date from {TABLE_ID}: {e}")

    default_start = date(2005, 1, 1)
    logger.info(f"No existing financial conditions data found; starting from {default_start}")
...
def main():
    ...
    if start_dt >= end_dt:
        logger.info("Financial conditions series already up to date; nothing to fetch.")
        return

    all_frames = []
    for sid in FIN_COND_SERIES:
        df = fetch_fred_series(sid, api_key, start_dt, end_dt)
        if not df.empty:
            all_frames.append(df)

    if not all_frames:
        logger.warning("No financial conditions data fetched from FRED.")
        return

    combined = pd.concat(all_frames, ignore_index=True)
    combined = combined.sort_values(["series_id", "date"]).drop_duplicates(["series_id", "date"])
    logger.info(f"Total financial conditions rows fetched this run: {len(combined)}")

    run_id = datetime.utcnow().strftime("%Y%m%d_%H%M%S")
    staging_dataset = f"{PROJECT_ID}.raw_staging"
    staging_table = f"{staging_dataset}.fred_financial_conditions_{run_id}"
    ...
    if not load_dataframe_to_table(combined, staging_table, PROJECT_ID, "WRITE_TRUNCATE"):
        logger.error(f"❌ Failed to load financial conditions series into staging table {staging_table}")
        return
    ...
    if merge_staging_to_target(
        ...
        logger.info(f"✅ Merged financial conditions series from {staging_table} into {TABLE_ID}")
    else:
