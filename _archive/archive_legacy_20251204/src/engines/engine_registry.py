...
class EngineRegistry:
    ...
    def update_performance(self, engine_name: str, mape: float):
        ...
        if engine_name in self.performance_history:
            ...
            if len(self.performance_history[engine_name]) > 100:
                self.performance_history[engine_name] = self.performance_history[engine_name][-100:]
    ...
    def get_weights(self, engine_names: Optional[List[str]] = None) -> Dict[str, float]:
        ...
        for name in engine_names:
            ...
            if name in self.performance_history and self.performance_history[name]:
                recent_mape = sum(self.performance_history[name][-10:]) / len(self.performance_history[name][-10:])
    ...
    def combine_forecasts(self, forecasts: Dict[str, pd.DataFrame], weights: Optional[Dict[str, float]] = None) -> pd.DataFrame:
        """
        ...
        for engine_name, forecast_df in forecasts.items():
            ...
            if weight > 0 and 'forecast' in forecast_df.columns:
                ...
                merged = combined.merge(forecast_df[['date', 'forecast']], on='date', how='outer', suffixes=('', f'_{engine_name}'))
                combined['forecast'] = combined['forecast'] + merged[f'forecast_{engine_name}'].fillna(0) * weight
