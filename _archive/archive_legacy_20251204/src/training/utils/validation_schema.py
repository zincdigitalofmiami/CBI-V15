...
training_data_schema = pa.DataFrameSchema({
    ...
    "target_1w_price": Column(float, nullable=True, checks=[
        Check(lambda x: x > 0, error="Target price must be positive")
    ]),
    "target_1m_price": Column(float, nullable=True, checks=[
        Check(lambda x: x > 0, error="Target price must be positive")
    ]),
    "target_3m_price": Column(float, nullable=True, checks=[
        Check(lambda x: x > 0, error="Target price must be positive")
    ]),
    "target_6m_price": Column(float, nullable=True, checks=[
        Check(lambda x: x > 0, error="Target price must be positive")
    ]),
    
    # Fundamental Spreads (Must be economically valid)
    "board_crush": Column(float, nullable=True, checks=[
        Check(lambda x: x > -100, error="Crush margin too negative (likely error)")
    ]),
    "oil_share": Column(float, nullable=True, checks=[
        Check(lambda x: (x >= 0) & (x <= 1), error="Oil share must be [0, 1]")
    ]),
    
    # Sentiment Features (CRITICAL: Logic Validation)
    "news_china_sentiment_net_7d": Column(float, nullable=True, checks=[
        # CRITICAL: China buying must be positively correlated with ZL returns
        # This check validates the logic isn't inverted
        Check(
            lambda df: df.corr(df['zl_return']) > -0.1 if 'zl_return' in df.columns else True,
            error="CRITICAL: China sentiment negatively correlated with ZL returns. Logic inverted?"
        )
    ]),
    
    "news_biofuel_sentiment_net_7d": Column(float, nullable=True, checks=[
    ...
    "rsi_14": Column(float, nullable=True, checks=[
        Check(lambda x: (x >= 0) & (x <= 100), error="RSI must be [0, 100]")
    ]),
    "bb_pct_b": Column(float, nullable=True, checks=[
        Check(lambda x: x >= -1, error="Bollinger %B too negative")
    ]),
    
    # Volatility (Must be positive)
    "vol_21d": Column(float, nullable=True, checks=[
        Check(lambda x: x >= 0, error="Volatility must be non-negative")
    ]),
    
    # Correlations (Must be [-1, 1])
    "corr_zl_brl_60d": Column(float, nullable=True, checks=[
        Check(lambda x: (x >= -1) & (x <= 1), error="Correlation must be [-1, 1]")
    ]),
    
    # Regime Weights (Must be positive)
    "regime_weight": Column(float, nullable=True, checks=[
        Check(lambda x: x > 0, error="Regime weight must be positive")
    ]),
...
def validate_feature_logic(df: pd.DataFrame, feature_name: str, expected_correlation: float = 0.1) -> bool:
    """
...
sentiment_feature_schema = pa.DataFrameSchema({
    ...
    "news_china_sentiment_net_7d": Column(float, nullable=True, checks=[
        Check(
            lambda df: validate_feature_logic(df, 'news_china_sentiment_net_7d', expected_correlation=0.1),
    ...
    "news_biofuel_sentiment_net_7d": Column(float, nullable=True, checks=[
        Check(
            lambda df: validate_feature_logic(df, 'news_biofuel_sentiment_net_7d', expected_correlation=0.1),
            error="Biofuel sentiment logic inverted"
        )
    ]),
    
    # Supply Sentiment (CRITICAL: Must be NEGATIVELY correlated - more supply = lower price)
    "news_supply_sentiment_net_7d": Column(float, nullable=True, checks=[
        Check(
            lambda df: df['news_supply_sentiment_net_7d'].corr(df['zl_return']) < 0.1 if 'zl_return' in df.columns else True,
