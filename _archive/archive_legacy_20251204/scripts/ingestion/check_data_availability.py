...
def main():
    ...
    logger.info(f"  Staging Layer: {'✅ Has data' if staging_has_data else '⚠️  No data (run Python/SQL staging transforms)'}")
    logger.info(f"  Features Layer: {'✅ Has data' if feature_has_data else '⚠️  No data (run Python feature builders)'}")
    
    logger.info("\n🎯 Next Steps:")
    if not raw_has_data:
        logger.info("  1. Run data ingestion: python3 src/ingestion/databento/collect_daily.py")
    elif not staging_has_data:
        logger.info("  1. Build staging tables via Python/SQL transforms")
    elif not feature_has_data:
        logger.info("  1. Run Python feature builders (for example src/features/build_daily_ml_matrix.py)")
    else:
