...
def check_ingestion_status():
    ...
    staging_tables = {
        ...
        "FRED Macro Panel": "staging.fred_macro_panel",
    ...
    for table_name, table_id in staging_tables.items():
        try:
            ...
            if count > 0:
                ...
                logger.info(f"  ⚠️  {table_name}: Empty (run Python/SQL staging transforms)")
        except Exception as e:
    ...
    try:
        ...
        if count > 0:
            ...
            logger.info(f"  ⚠️  Daily ML Matrix: Empty (run Python feature builders)")
    except Exception as e:
    ...
    logger.info("  - Run Python feature pipeline (build_daily_ml_matrix.py + load_daily_ml_matrix.py)")
