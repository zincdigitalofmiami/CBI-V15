...
--
-- PARTITIONING STRATEGY (per quant best practices):
-- - Multi-symbol, multi-year tables: PARTITION BY DATE_TRUNC(date, MONTH)
-- - Single-series tables (FRED, USDA): PARTITION BY DATE(date) - daily is fine
-- - NO TTL on canonical history (raw, staging, features, training, forecasts)
-- - TTL only on ops/logs/temp tables

-- ============================================================================
-- RAW LAYER
-- ============================================================================

-- Market Data (Databento) - MULTI-SYMBOL, needs MONTH partitioning
...
-- Market Data (Databento) - 1-hour OHLCV - MULTI-SYMBOL, needs MONTH partitioning
CREATE TABLE IF NOT EXISTS `cbi-v15.raw.databento_futures_ohlcv_1h` (
  date DATE,
  hour INT64,
  symbol STRING,
  open FLOAT64,
...
-- Market Data (Databento) - Statistics messages (per-event)
CREATE TABLE IF NOT EXISTS `cbi-v15.raw.databento_futures_statistics` (
  date DATE,
  symbol STRING,
  stat_type STRING,
  price FLOAT64,
...
-- FRED Macro Panel
CREATE TABLE IF NOT EXISTS `cbi-v15.staging.fred_macro_panel` (
  date DATE,
  series_id STRING,
  value FLOAT64,
  forward_filled BOOL
...
-- USDA Reports Panel
CREATE TABLE IF NOT EXISTS `cbi-v15.staging.usda_reports_panel` (
  report_date DATE,
  report_type STRING,
  commodity STRING,
  metric STRING,
...
-- EIA Biofuels Panel
CREATE TABLE IF NOT EXISTS `cbi-v15.staging.eia_biofuels_panel` (
  date DATE,
  series_id STRING,
  value FLOAT64
)
...
-- Daily ML Matrix (Master Join Table) - DENSE, needs MONTH partitioning
...
-- ZL Training Tables (structure will be populated by Dataform) - MONTH partitioning
...
-- ZL Predictions Tables - MONTH partitioning for backtesting history
