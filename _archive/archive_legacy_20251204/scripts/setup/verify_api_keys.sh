...
echo "📱 macOS Keychain:"
KEYS=("DATABENTO_API_KEY" "SCRAPECREATORS_API_KEY" "FRED_API_KEY")
for key in "${KEYS[@]}"; do
...
SECRET_COUNT=$(gcloud secrets list --project="$PROJECT_ID" --filter="name:databento-api-key OR name:scrapecreators-api-key OR name:fred-api-key" --format="value(name)" 2>/dev/null | wc -l | tr -d ' ' || echo "0")
