 ...
 * 
 * Scrapes ALL ProFarmer URLs daily:
 * - First Thing Today
 * - Ahead of the Open
 * - After the Bell
 * - Agriculture News
 ...
 * Each URL is critical and must be pulled daily.
 */

import { task, schedules } from "@trigger.dev/sdk/v3";
import { MotherDuckClient } from "../src/shared/motherduck_client";

// ALL ProFarmer URLs to scrape
const PROFARMER_URLS = {
  // Daily Editions (CRITICAL)
  daily_editions: [
...
async function scrapeProFarmerURL(
  ...
  try {
    ...
    // Scrape full content (500 words max)
...
export const profarmerAllURLs = task({
  ...
  run: async (payload: { priorities?: string[] }, { ctx }) => {
    ...
    console.log(`[ProFarmer All URLs] Scraping ${urlsToScrape.length} URLs...`);
    ...
    for (const urlConfig of urlsToScrape) {
      try {
        ...
        console.log(`[ProFarmer] Found ${articles.length} articles`);
      } catch (error) {
    ...
    if (allArticles.length > 0) {
      ...
      console.log(`[ProFarmer] Loaded ${allArticles.length} articles to MotherDuck`);
    }
