...
export const eiaProcurementIngest = task({
  ...
  run: async (payload: { daysBack?: number }, { ctx }) => {
    ...
    // Load biofuel feedstock series to MotherDuck (raw.eia_biofuels)
    if (allRecords.length > 0) {
      const motherduck = new MotherDuckClient();

      const biofuelRecords = allRecords
        .filter(r => r.category === "biofuel_feedstock")
        .map(r => ({
          date: r.date,
      ...
      if (biofuelRecords.length === 0) {
        console.log("[EIA] No biofuel feedstock records to load into raw.eia_biofuels");
        await motherduck.close();
      } else {
        await motherduck.insertBatch("raw.eia_biofuels", biofuelRecords);
        await motherduck.close();
        console.log(`[EIA] Loaded ${biofuelRecords.length} biofuel records to raw.eia_biofuels`);
      }
    }
